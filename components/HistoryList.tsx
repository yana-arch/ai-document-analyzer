...
const HistoryList: React.FC<HistoryListProps> = memo(({ items, onLoadItem, onImportHistory, onSyncToDatabase }) => {
  const { t } = useLanguage();
  ...
  // Removed automatic fetching of DB items as per user request
    ...
    <div className="max-w-6xl mx-auto mt-6 sm:mt-8 lg:mt-16 mobile-container">
      ...
      >
        ...
        <div className="flex justify-center gap-3 sm:gap-4 pt-4 border-t border-zinc-200 dark:border-zinc-700 flex-wrap sm:flex-nowrap">
          ...
          {onSyncToDatabase && filteredAndSortedItems.length > 0 && (
            <button
              onClick={onSyncToDatabase}
              className="px-3 sm:px-4 py-2 bg-green-600 text-white rounded-lg hover:bg-green-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-green-500 transition-colors flex items-center gap-2 touch-target mobile-text"
            >
              <svg width="14" height="14" className="sm:w-4 sm:h-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
                <path d="M21 15v4a2 2 0 01-2 2H5a2 2 0 01-2-2v-4"/>
                <polyline points="7,10 12,15 17,10"/>
              ...
              <span className="hidden sm:inline">{t('history.syncButton') || 'Sync to DB'}</span>
